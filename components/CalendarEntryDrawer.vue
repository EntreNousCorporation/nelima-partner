...
import {
    MONTHS, collectionRate, kindLabel, kindTone, type CalendarEntry,
...
<template>
    <SideDrawer
        :title="entry.title"
        :sub="`${kindLabel(entry.kind)} · ${dateLabel}`"
        @close="emit('close')"
    >
        <template #avatar>
            <div
                class="w-11 h-11 rounded-xl shrink-0"
                :style="{ background: 'var(--surface-sunken)', borderLeft: `4px solid ${kindTone(entry.kind)}` }"
            />
        </template>
        ...
        <dl class="rounded-xl overflow-hidden mb-5" style="border: 1px solid var(--border)">
            <div class="flex px-3 py-2" style="border-bottom: 1px solid var(--border)">
                ...
                <dd class="text-[12.5px] font-semibold">{{ kindLabel(entry.kind) }}</dd>
            </div>
            <div class="flex px-3 py-2" style="border-bottom: 1px solid var(--border)">
